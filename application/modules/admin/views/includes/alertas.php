...
if($alerta){
	...
	?>
	<div class="alert <?php echo isset($alerta["type"]) && !empty($alerta["type"]) ? 'alert-' . $alerta["type"] : 'alert-danger'; ?> <?php echo isset($alerta["class"]) && !empty($alerta["class"]) ? (is_array($alerta["class"]) ? implode(" ", $alerta["class"]) : $alerta["class"]) : null; ?> <?php echo isset($alerta["status"]) && $alerta["status"] == 'visible' ? '' : 'display-hide'; ?>" role="alert">
